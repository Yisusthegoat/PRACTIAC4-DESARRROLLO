(() => {
  ...
  type PaletteName = "aurora" | "ocean" | "sunset";
  type ShapeName = "hummingbird" | "whale" | "fox";
  ...
  const fpsValue = document.querySelector<HTMLSpanElement>("#fpsValue");
  const memoryUsed = document.querySelector<HTMLElement>("#memoryUsed");
  const memoryLimit = document.querySelector<HTMLElement>("#memoryLimit");
  const memoryStatus = document.querySelector<HTMLParagraphElement>("#memoryStatus");
  const particleCounter = document.querySelector<HTMLSpanElement>("#particleCounter");
  const statusBadge = document.querySelector<HTMLSpanElement>("#statusBadge");
  const particleAmount = document.querySelector<HTMLInputElement>("#particleAmount");
  const speedAmount = document.querySelector<HTMLInputElement>("#speedAmount");
  const particleSize = document.querySelector<HTMLInputElement>("#particleSize");
  const shapeSelect = document.querySelector<HTMLSelectElement>("#shapeSelect");
  const paletteSelect = document.querySelector<HTMLSelectElement>("#paletteSelect");
  const trailToggle = document.querySelector<HTMLInputElement>("#trailToggle");
  const validationMessage = document.querySelector<HTMLParagraphElement>("#validationMessage");
  const actionButtons = document.querySelector<HTMLDivElement>("#actionButtons");
  const pointerStatus = document.querySelector<HTMLElement>("#pointerStatus");
  const particleAmountValue = document.querySelector<HTMLOutputElement>("#particleAmountValue");
  const speedAmountValue = document.querySelector<HTMLOutputElement>("#speedAmountValue");
  const particleSizeValue = document.querySelector<HTMLOutputElement>("#particleSizeValue");

  if (!canvas || !fpsValue || !memoryUsed || !memoryLimit || !memoryStatus || !particleCounter || !statusBadge || !particleAmount || !speedAmount || !particleSize || !shapeSelect || !paletteSelect || !trailToggle || !validationMessage || !actionButtons || !pointerStatus || !particleAmountValue || !speedAmountValue || !particleSizeValue) {
    return;
  }
  ...
  const pointerStatusDisplay = pointerStatus;
  const particleAmountDisplay = particleAmountValue;
  const speedAmountDisplay = speedAmountValue;
  const particleSizeDisplay = particleSizeValue;
  ...
  const readOptions = (): EngineOptions => ({
    ...
    shape: shapeInput.value as ShapeName
  });

  const updateControlReadouts = (): void => {
    particleAmountDisplay.value = particleInput.value;
    speedAmountDisplay.value = speedInput.value;
    particleSizeDisplay.value = sizeInput.value;
  };

  function addEllipse(points: Array<{ x: number; y: number }>, cx: number, cy: number, rx: number, ry: number, count: number): void {
  ...
  function addLine(points: Array<{ x: number; y: number }>, x1: number, y1: number, x2: number, y2: number, count: number): void {
    for (let i = 0; i < count; i += 1) {
  ...
  function addArc(points: Array<{ x: number; y: number }>, cx: number, cy: number, rx: number, ry: number, start: number, end: number, count: number): void {
    for (let i = 0; i < count; i += 1) {
      const t = count === 1 ? 0 : i / (count - 1);
  ...
  function buildShapePoints(width: number, height: number, shape: ShapeName, count: number): Array<{ x: number; y: number }> {
    ...
    while (points.length < count) {
      points.push(points[points.length % Math.max(points.length, 1)] ?? { x: cx, y: cy });
    }

    return points.slice(0, count);
  }

  function createParticle(width: number, height: number, options: EngineOptions): Particle {
  ...
  function setTheme(palette: PaletteName): void {
  ...
  function validateOptions(options: EngineOptions): boolean {
  ...
  function applySpeedToParticle(particle: Particle, speed: number): void {
  ...
  function applySizeToParticle(particle: Particle, size: number): void {
  ...
  function createParticleEngine(renderContext: CanvasRenderingContext2D, renderCanvas: HTMLCanvasElement) {
    ...
    */
    ...
    const updateOptions = (nextOptions: EngineOptions): void => {
    ...
    const setPointer = (x: number, y: number, active: boolean): void => {
      pointer.x = x;
      pointer.y = y;
  ...
  const handlePointerMove = (event: PointerEvent): void => {
    ...
    pointerStatusDisplay.textContent = "Campo activo";
  };
  ...
  canvasElement.addEventListener("pointerleave", () => {
    ...
    pointerStatusDisplay.textContent = "Explora el lienzo";
  });
